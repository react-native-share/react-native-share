...
- (void)backgroundVideo:(NSData *)backgroundVideo  stickerImage:(NSData *)stickerImage
{
    // Verify app can open custom URL scheme. If able,
    // assign assets to pasteboard, open scheme.
    NSURL *urlScheme = [NSURL URLWithString:@"instagram-stories://share"];
    if ([[UIApplication sharedApplication] canOpenURL:urlScheme]) {
        // Assign background image asset and attribution link URL to pasteboard
        NSArray *pasteboardItems = @[@{@"com.instagram.sharedSticker.backgroundVideo" : backgroundVideo, @"com.instagram.sharedSticker.stickerImage" : stickerImage}];
...
- (void)shareSingle:(NSDictionary *)options
    ...
    NSString *method = [RCTConvert NSString:options[@"method"]];
       RCTLog(@"method method is %@", method);
    ...
    if (method) {
        ...
        if([method isEqualToString:@"shareBackgroundImage"]) {
            ...
            if (URL == nil || stickerURL == nil) {
                RCTLogError(@"key 'backgroundVideo' or 'sticker' missing in options");
            } else {
                ...
                [self backgroundVideo: backgroundVideo stickerImage:UIImagePNGRepresentation(stickerImage)];
            }
        }
    } else {
