...
**Note:** Base64 attachments are prepared in app-specific cache storage. Android 4.4
(API 19) and later do not require storage permissions for these directories; do not
request `WRITE_EXTERNAL_STORAGE` just to share base64 data. Reading a source file
outside your app's storage may still require access granted by its provider.
See [app-specific storage](https://developer.android.com/training/data-storage/app-specific).
...
The included Windows implementation uses a legacy callback-based native `open`
method, while the current JavaScript API expects a promise. Windows compatibility
needs validation before use; the former C# `RNSharePackage` instructions do not
match the included C++/WinRT package provider. Android and iOS installation steps
above do not establish Windows support.
